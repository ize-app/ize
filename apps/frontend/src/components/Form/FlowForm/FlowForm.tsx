...
export const FlowForm = forwardRef(({ name, isReusable }: FlowFormProps, ref) => {
  const { formState, setFormState, onNext } = useNewFlowWizardState();
  ...
  const useFormMethods = useForm<FlowSchemaType>({
    defaultValues: {
      ...
      trigger: formState.new[name].trigger ?? {
        permission: {
