...
const getEvolveDiffForDisplay = (
  ...
  if (processDiff.customOptions || processDiff.options) diffFields.push(DiffField.Options);
  ...
  if (processDiff.evolve?.rights?.request) diffFields.push(DiffField.EvolveRequest);
  if (processDiff.evolve?.rights?.response) diffFields.push(DiffField.EvolveRespond);
